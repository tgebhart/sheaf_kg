...
from sheafE_models import SheafE_Multisection_Complex_Queries
...
def run(dataset, dataset_loc, num_epochs, batch_size, embedding_dim, edge_stalk_dim, loss, training_loop,
    ...
    model = SheafE_Multisection_Complex_Queries(num_entities, num_relations, **model_kwargs)
    ...
    for epoch in range(1,num_epochs+1):
        ...
        for bix in tqdm(range(0, num_queries, batch_size)):
            ...
            for param in params:
                ...
                if g != g:
                    print('param norms', g)
        print('Train Loss: {:.2f}'.format(bix, loss.item()))
        params = model.get_parameters()
        for param in params:
            g = param.grad.norm()
            print('outside batch loop param norms', g)
...
if __name__ == '__main__':
    ...
    training_args.add_argument('--edge-stalk-dim', type=int, default=edge_stalk_dim,
                        help='entity embedding dimension')
    ...
    run(args.dataset, args.dataset_loc, args.num_epochs, args.batch_size, args.embedding_dim, args.edge_stalk_dim, args.loss,
        args.training_loop, args.seed, args.num_sections, args.symmetric,
        args.orthogonal, args.alpha_orthogonal, args.scoring_fct_norm, args.model_inverses)
